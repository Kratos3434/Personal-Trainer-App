    ...
    for (const muscleGroupId of muscleGroupIds) {
      for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        try {
          ...
          console.log(
            `Junction created for exercise ID: ${exerciseId} and muscle group ID: ${muscleGroupId}`,
          );
          ...
          if (attempt === 3) {
            console.error(
              `Failed after 3 attempts for muscle group ID ${muscleGroupId}:`,
              err.message,
            );
            throw err;
          }
...
async function createEnvironmentJunctions(exerciseId, workoutEnvironmentIds) {
  try {
    for (const workoutEnvironmentId of workoutEnvironmentIds) {
      for (let attempt = 1; attempt <= 3; attempt++) {
        // Retry loop because of potential throttling that caused import failures
        try {
          const junctionData = {
            exerciseId: exerciseId,
          ...
          console.log(
            `Junction created for exercise ID: ${exerciseId} and workout environment ID: ${workoutEnvironmentId}`,
          );
          await new Promise((resolve) => setTimeout(resolve, 500)); // Delay
          break; // Exit loop on success
        } catch (err) {
          if (attempt === 3) {
            console.error(
              `Failed after 3 attempts for environment ID ${workoutEnvironmentId}:`,
              err.message,
            );
            throw err;
          }

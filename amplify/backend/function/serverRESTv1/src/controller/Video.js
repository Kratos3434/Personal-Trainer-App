...
class Video {
    ...
    static async getVideosFromYoutubeByExerciseName(name, useNewKey = false) {
        try {
            ...
            const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&q=${encodeURIComponent(query)}&videoDuration=${videoDuration}&order=relevance&maxResults=${maxResults}&key=${apiKey}`);

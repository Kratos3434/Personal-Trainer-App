...
async function assessProgress(currentMeasurementId, prevMeasurementId) {
  if (!currentMeasurementId || !prevMeasurementId)
    throw "IDs for both body measurements are required";
...
class WeeklyProgress {
  ...
  static async saveWeeklyProgress(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ status: true, data: result, message: "Weekly Progress saved successfully" });
    } catch (err) {
  ...
  static async getProgressById(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ status: true, data: progress, message: "Progress entry retrieved successfully!" });
    } catch (err) {
      res.status(400).json({ status: false, error: err });
    }
  }
  ...
  static async getProgressResults(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ status: true, data: result, message: "Progress results retrieved successfully!" });
    } catch (err) {
      res.status(400).json({ status: false, error: err });
    }
  }

...
class Algorithm {
  ...
  static async getRecommendation(req, res) {
    ...
    try {
      ...
      const bodyFatClassification = FitnessUtils.getClassificationResult(
        latestMeasurement.bodyFatPercent,
        age,
        profile.gender,
      );
      const ffmi = FitnessUtils.getFFMI(
        profile.height,
      ...
      const maxExercisesPerDay =
        intensityId === 1 ? 4 : intensityId === 2 ? 5 : intensityId === 3 ? 6 : 7;
      ...
      if (["Average", "Below Average", "Poor"].includes(bodyFatClassification)) {
        selectedBasePlan = Algorithm.basePlans.find(
          (plan) => plan.title === "Fat Loss" && plan.gender === gender,
        );
      ...
      if (latestProgress.length > 0) {
        ({ bodyFatDecreased, bodyMassIncreased } =
          await Algorithm.calculateBodyFatAndBodyMassChanges(
            latestProgress,
      ...
      for (let i = 0; i < selectedDays.slice(0, daysPerWeek).length; i++) {
        ...
        for (const exercise of day.exercises) {
          ...
          while (maxTries > 0) {
            ...
            if (Array.isArray(exercises) && exercises.length > 0) {
              ...
              if (filteredExercises.length > 0) {
                ...
                // Get video
                const videoData = await Video.getByExerciseIdRandom({
                  exerciseId: selectedExercise.id,
                });
                const youtubeURL = videoData?.data?.url || `URL not found: ${videoData.exerciseId}`;
                const thumbnailURL =
                  videoData?.data?.thumbnail || `Thumbnail not found: ${videoData.exerciseId}`;

                rawExerciseDetails.push({
                  exerciseId: selectedExercise.id,
                  sets: selectedExercise.defaultSets,
                  reps: modifyReps
                    ? selectedExercise.defaultReps + 2
                    : selectedExercise.defaultReps, // If modifyReps is true, increase 2 reps
                  minutes:
                    selectedExercise.minutes && modifyCardio
                      ? selectedExercise.minutes * 2
                      : selectedExercise.minutes, // if modify cardio is true, x2 minutes
            ...
            if (maxTries === 5) {
              ...
              console.log(
                "First attempt: No matching exercises found, switching typeId: ",
                exerciseCriteria,
              );
              ...
              console.log(
                "Second attempt: No matching exercises found, lowering levelId: ",
                exerciseCriteria,
              );
            } else if (maxTries === 3) {
              exerciseCriteria.typeId = exerciseCriteria.typeId === 1 ? 2 : 1;
              console.log(
                "Third attempt: No matching exercises found, switching typeId: ",
                exerciseCriteria,
              );
              ...
              exerciseCriteria.levelId -= 1;
              console.log(
                "Fourth attempt: No matching exercises found, lowering levelId: ",
                exerciseCriteria,
              );
            } else if (maxTries === 1 && exerciseCriteria.levelId > 1) {
              exerciseCriteria.typeId = exerciseCriteria.typeId === 1 ? 2 : 1;
              console.log(
                "Fifth attempt: No matching exercises found, switching typeId: ",
                exerciseCriteria,
              );
            }
            maxTries--;
          ...
          if (!foundExercise) {
            console.log(
              `Failed fetching exercises: No matching exercises found matching the criteria, this exercise will be omitted:`,
              exerciseCriteria,
            );
        ...
        const cardioExercises = filteredExerciseDetails.filter((ex) =>
          ex.muscleGroups.includes(12),
        );
        const nonCardioExercises = filteredExerciseDetails.filter(
          (ex) => !ex.muscleGroups.includes(12),
        );

        // Merge two arrays into one
        const exerciseDetails = [
          ...nonCardioExercises.slice(0, maxExercisesPerDay),
          ...cardioExercises,
        ];
      ...
      if (bodyFatDecreased !== undefined && !bodyFatDecreased)
        Polling.addLog(`Need to put more work on fat loss, adding/modifying cardio exercise...\n`);
      if (bodyMassIncreased !== undefined && !bodyMassIncreased)
        Polling.addLog(`Need to put more work on building muscle, adding reps to exercises...\n`);
      ...
      if (
        bodyFatDecreased !== undefined &&
        bodyFatDecreased &&
        bodyMassIncreased !== undefined &&
        !bodyMassIncreased
      ) {
        Polling.addLog(
          `Warning: Both of your body fat and body mass are declining the past weeks, you might have overtrained, we suggest you to take a break for a week.\n`,
        );
      }
   ...
   * @returns
   */
  static async calculateBodyFatAndBodyMassChanges(
    latestProgress,
    bodyFatClassification,
    ffmiClassification,
  ) {
    ...
    if (
      lastFourMeasurements.length > 2 &&
      ["Average", "Below Average", "Poor"].includes(bodyFatClassification)
    ) {
      const bodyFatPercentSum = lastFourMeasurements.reduce(
        (sum, measurement) => sum + measurement.bodyFatPercent,
        0,
      );
    ...
    if (
      lastFourMeasurements.length > 2 &&
      ["Skinny", "Average", "Intermediate Built"].includes(ffmiClassification)
    ) {
      const bodyMassSum = lastFourMeasurements.reduce(
        (sum, measurement) => sum + measurement.muscleMass,
        0,
      );
